...
#define FTNewFeatureCount 4


@interface FTNewFeatureViewController () <UIScrollViewDelegate>
...
@implementation FTNewFeatureViewController
...
- (void)viewDidLoad {
    ...
    for (int i = 0; i<FTNewFeatureCount; i++) {
...
#warning 就算不主动添加子控件到scrollView中，scrollView内部还是可能会有一些子控件
    ...
    scrollView.contentSize = CGSizeMake(FTNewFeatureCount * scrollW, 0);
    ...
    pageControl.numberOfPages = FTNewFeatureCount;
    pageControl.backgroundColor = [UIColor redColor];
    pageControl.currentPageIndicatorTintColor = FantasyColor(253, 98, 42);
    pageControl.pageIndicatorTintColor = FantasyColor(189, 189, 189);
...
{
    // 其实就是利用scrollView去控制pageControl
...
- (void)startClick
{
    ...
    window.rootViewController = [[FantasyTabBarViewController alloc] init];
